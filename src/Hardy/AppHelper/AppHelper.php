...
class AppHelper {
    ...
    public static function UrlRoute($is_dir = false, $name, $parameters = array(), $absolute = true, $route = null) {
        $root = URL::route($name, $parameters, $absolute, $route) . ($is_dir ? '/' : Config::get('app.url_suffix', ''));
        return AppHelper::removeIndex($root);
    }
    ...
    public static function Locations() {
        ...
        $locations['provices'] = Location::provinces()->get()->toJson();
        $locations['cities'] = Location::allcities()->get()->toJson();
        $locations['districts'] = Location::alldistricts()->get()->toJson();
    ...
    public static function wapGetOrderAction($order, $withCancel = true) {
        ...
        switch ($order['status']) {
            case 'approved':
                $action = HTML::link(URL::route('wap_payOrder', array('id' => $order['orderId'])), '支付', array('class' => 'order-action confirm-buy', 'target' => '_blank'));
                break;
            case 'received':
                $action = HTML::link(URL::route('wap_addComment', array('id' => $order['orderId'])), '评价', array('class' => 'order-action'));
                break;
            case 'approved':
                $action = HTML::link(URL::route('wap_payOrder', array('id' => $order['orderId'])), '支付', array('class' => 'order-action'));
                break;
            case 'approved':
                $action = HTML::link(URL::route('wap_payOrder', array('id' => $order['orderId'])), '支付', array('class' => 'order-action'));
                break;
        ...
        if (in_array($order['status'], array('requested', 'approved', 'reject_approval')) && $withCancel) {
            $action .= HTML::link(URL::route('wap_cancelOrder', array('id' => $order['orderId'])), '取消订单', array('class' => 'order-action'));
        }
        return $action;
    }
